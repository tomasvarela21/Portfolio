...
export default function CertificationsLearning({}: Record<string, never> = {}) {
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card className="border-neutral-800 bg-neutral-900/60 text-neutral-200">
        ...
        <CardContent>
          <ul className="space-y-3">
            {translations[language].trainings.map((t, index) => (
              <li key={index} className="flex items-start gap-3">
                <span className="mt-1 h-2.5 w-2.5 rounded-full bg-neutral-500" aria-hidden />
                <div>
                  <p className="font-medium text-neutral-100">{t.title}</p>
                  <p className="text-sm text-neutral-400">{t.provider}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <Card className="border-neutral-800 bg-neutral-900/60 text-neutral-200">
        ...
        <CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {stackIcons.learning.map((t) => (
            <div
              key={t.label}
              className="flex items-center gap-2 rounded-md border border-neutral-800 bg-neutral-950/60 p-2"
            >
              <img src={t.src || "/placeholder.svg"} alt={`${t.label} logo`} className="h-6 w-6 object-contain" />
